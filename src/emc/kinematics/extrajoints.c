...
int extrajoints_setup (int  kinsjoints,
    ...
    if (extrajoints < 0) {extrajoints = 0;} // silently ignore
    if (extrajoints > MAX_EXTRA_JOINTS) {
        rtapi_print_msg(RTAPI_MSG_ERR,
             "EXTRAJOINTS: too many extrajoints:%d limit=%d\n",
             extrajoints,MAX_EXTRA_JOINTS);
        return -1;
    }
    if ( (extrajoints + kinsjoints) > EMCMOT_MAX_JOINTS) {
        rtapi_print_msg(RTAPI_MSG_ERR,
             "EXTRAJOINTS: too many total joints:%d limit=%d\n",
             extrajoints+kinsjoints,EMCMOT_MAX_JOINTS);
        return -1;
    }
